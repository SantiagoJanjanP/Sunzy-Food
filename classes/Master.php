...
Class Master extends DBConnection {
    ...
    if($check > 0){
        ...
        if($save){
            ...
            if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
                ...
                $uploadFile = base_app.$uploadDir . basename($_FILES['img']['name']);

                $imageFileType = strtolower(pathinfo($uploadFile,PATHINFO_EXTENSION));
                $targetFile = $uploadDir . $pid . '.' . $imageFileType;
                ...
                if($check !== false) {
                    ...
                    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
                        $resp['msg'] = "Sorry, only JPG, JPEG, PNG files are allowed.";
                    } else {
                        if (move_uploaded_file($_FILES["img"]["tmp_name"], base_app.$targetFile)) {
                            $this->conn->query("UPDATE product_list set image_path = '{$targetFile}' where id = '$pid'");
                        } else {
            ...
            $resp['status'] = 'failed';
            if(empty($id))
                $resp['msg'] = " Service has failed to save.";
            else
                $resp['msg'] = " Service has failed to update.";
            $resp['err'] = $this->conn->error."[{$sql}]";
        }
    }
	...
	function add_to_cart(){
		...
		if($save){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = " The Service has failed to add to the cart.";
	...
	function update_cart_qty(){
		...
		if($update_cart){
			$resp['status'] = 'success';
			$resp['msg'] = ' Service has updated successfully';
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = ' Service has failed to update';
			$resp['error'] = $this->conn->error;
	...
	function place_order(){
		...
		if(!$has_failed){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = " Service has failed to place";
			$resp['error'] = $this->conn->error;
			if(count($inserted) > 0){
	...
	function update_status(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " Service Status has been updated successfully.";
		}else{
			$resp['status'] = 'success';
			$resp['msg'] = " Service Status has failed to update.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['msg']);
